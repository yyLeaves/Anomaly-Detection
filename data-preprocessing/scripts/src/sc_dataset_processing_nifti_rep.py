...
from utils.processing_utils import (
    load_scan,
    extract_3ch_slice_rep, # Logic: Same slice copied to 3 channels
...
# ================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Repeated 3-channel NIfTI pelvis preprocessing")
...
def process_slices(
    ...
    """Process slices using repeated (i, i, i) channels into 3-channel NIfTI."""
    ...
    for i in tqdm(slice_indices, desc=f"{id_}-{split}-{subset}"):
        ...
        # 3. Process 3 Channels (though they are identical here)
        processed_channels = []
        for c in range(3):
...
if __name__ == "__main__":
    ...
    logger.info("✅ Dataset Creation Complete (Repeated NIfTI).")