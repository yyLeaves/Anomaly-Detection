...
def _radimagenet_resnet_sequential_to_named(sd: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    """Convert keys saved from a Sequential(list(resnet.children())[:-1]) into standard resnet names.

    Expected input keys look like:
      ...
      - 'backbone.6.*'           -> 'layer3.*'
      - 'backbone.7.*'           -> 'layer4.*'
    We first strip 'backbone.' then remap the numeric prefixes.
    """
    ...
    for k, v in sd.items():
        ...
        if nk.startswith("0."):
            ...
            nk = "layer4." + nk[len("7."):]
        # skip avgpool(8) / fc(9) by default, as anomaly models generally use features only
...
def load_radimagenet_resnet_weights(module: torch.nn.Module, weights_path: str, strict: bool = False) -> Tuple[list, list]:
    """Load RadImageNet ResNet weights saved as OrderedDict with 'backbone.'-prefixed sequential keys.

    - Strips 'backbone.' prefix
    - Remaps numeric indices to standard resnet names (conv1/bn1/layers)
    - Loads into the provided module (should expose resnet-like submodules)
