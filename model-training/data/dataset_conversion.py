"""
Utilities for preparing datasets (PNG or NIfTI) for Anomalib models.
Converts input NIfTI volumes into PNG images (mirroring folder structure).
"""
...
# Optional: replace these with your own utility implementations if needed
def canonical_suffix(path: Path) -> str:
    """Return lowercase file suffix, handling .nii.gz."""
    s = path.suffix.lower()
    if s == ".gz" and path.name.endswith(".nii.gz"):
        return ".nii.gz"
    return s

def normalise_for_display(data: np.ndarray) -> np.ndarray:
    """Normalize array to [0, 1] for display purposes."""
    data = np.asarray(data)
    dmin, dmax = np.nanmin(data), np.nanmax(data)
    if dmax > dmin:
        norm = ((data - dmin) / (dmax - dmin)).clip(0, 1)
    else:
...
def _normalise_image_array(data: np.ndarray) -> np.ndarray:
    ...
    if data.ndim == 1:
        return normalise_for_display(data[np.newaxis, :])[0]
    if data.ndim == 2:
...
def _convert_nifti_file(source: Path, destination: Path) -> None:
    ...
    payload = _normalise_mask_array(data) if is_mask else _normalise_image_array(data)
...
def convert_nifti_tree_to_png(source_root: Path, dest_root: Path) -> None:
    """
    Mirror a NIfTI dataset tree as PNG images for Anomalib training.
    """
...
def prepare_dataset_root(
    ...
    """
    Returns (prepared_root, converted) for Anomalib-compatible PNG tree.
    If NIfTI found, mirrors tree as PNGs in cache dir.
    """
