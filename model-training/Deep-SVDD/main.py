...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['RESC', 'OCT2017', 'liver', 'bras2021', 'chest', 'camelyon', 'custom']))
...
@click.option('--config_path', type=click.Path(exists=True), default=None,
              help='Override YAML config path (required for custom dataset).')
...
def main(dataset_name, net_name, xp_path, data_path, config_path, load_config, load_model, device, pretrain, n_jobs_dataloader, normal_class):
    """
    ...
    # Datasets
    print(dataset_name)
    _dr = data_path if dataset_name == 'custom' else None
    train_dataset = TrainDataset(data=dataset_name, transform=get_train_transforms(), data_root=_dr)
    valid_dataset = ValidDataset(data=dataset_name, transform=get_valid_transforms(), data_root=_dr)
    test_dataset  = TestDataset(data=dataset_name, transform=get_valid_transforms(), data_root=_dr)
