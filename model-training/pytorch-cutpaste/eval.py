...
def get_train_embeds(model, size, defect_type, transform, device, data_root=None):
    # train data / train kde
    _dr = data_root if defect_type == 'custom' else None
    train_data = TrainDataset(data=defect_type, transform=transform, data_root=_dr)
...
def eval_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch(), data_root=None):
    ...
    _dr = data_root if defect_type == 'custom' else None
    test_data_eval = ValidDataset(data=defect_type, transform=test_transform, data_root=_dr)
    ...
    if train_embed is None:
        train_embed = get_train_embeds(model, size, defect_type, test_transform, device, data_root=data_root)
...
def test_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch(), data_root=None):
    # create test dataset
    ...
    test_transform.transforms.append(transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                        std=[0.229, 0.224, 0.225]))
    _dr = data_root if defect_type == 'custom' else None
    test_data_eval = TestDataset(data=defect_type, transform=test_transform, data_root=_dr)
